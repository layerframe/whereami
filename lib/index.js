...
module.exports = {
   ...
   * @return {void}
   */
  init: function init(staging, production, develop) {
    var fStaging = staging || process.env.STAGING_URL || process.env.STAGE_URL || process.env.NEXT_PUBLIC_STAGING_URL;
    var fProduction = production || process.env.LIVE_URL || process.env.PRODUCTION_URL || process.env.NEXT_PUBLIC_PRODUCTION_URL || process.env.NEXT_PUBLIC_LIVE_URL;
    var fDevelop = develop || process.env.DEVELOP_URL || process.env.DEVELOPMENT_URL || process.env.NEXT_PUBLIC_DEVELOPMENT_URL || null;
  ...
  now: function now() {
    ...
    if (process.browser && !isServer) {
      ...
      if (process.env.NODE_ENV === 'production') {
        ...
        var isDevelop = false;
        isStaging = onStaging();
        isDevelop = onDev(); // Update the host

        host = isDevelop ? "".concat(protocol).concat(developUrl) : isStaging ? "".concat(protocol).concat(stagingUrl) : "".concat(protocol).concat(productionUrl);
      }
